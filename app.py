...
def main():
    ...
    # Initialize Q&A
    if username:
        chat("test", avatar_style="bottts", seed="Buster")
        chat("Another test", is_user=True, avatar_style="initials", seed=username)
        ...
        if st.session_state.index == 0:
            ...
            if audio_bytes:
                ...
                message = message1.text_area(label="Me",
                                             ...
                                             key=st.session_state.input_message_key + '2')
        ...
        if st.session_state.index == 2:
            ...
            if audio_bytes:
                ...
                stt = openai.Audio.transcribe("whisper-1", recording)
                st.session_state.responses[st.session_state.index] = stt["text"]
                message = message3.text_area(label="Me",
                                             label_visibility="collapsed",
                                             value=st.session_state.responses[st.session_state.index],
                                             key=st.session_state.input_message_key + '4')

            if message:
                submit = submit3.button("Send", type="primary")
